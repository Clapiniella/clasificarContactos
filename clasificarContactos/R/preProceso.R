...
preProcesarDatos <- function(datos, config){
  ...
  columnasUtilizadas <- c(config$columnas$ID, config$columnas$predictorasNumericas,
                          ...
                          config$columnas$fechas$apertura_primero, config$columnas$fechas$envio_ultimo,
  ...
  if(!check){
    ...
    columnasMalas <- names(columnasUtilizadas)[!checkColumnas]
  ...
  datos[ is.na(datos[, config$columnas$mails$mailsOp]), config$columnas$mails$mailsDl] <- 0
...
#' @examples
 ...
 #restar columnas de fechas y hacer as date para que no salga como character
#Hacer dummies de las columnas
...
openNegativos <- c("Contacto No Válido", "Descartado - Demasiados Intentos")
...
listaDatosSpliteados <- list(contactosPredict = contactosPredict, contactosEntrenados = contactosEntrenados)
